...
\begin{table*}[p]
  % table caption is above the table
  \caption{Comparison of event duration statistics for image, dot, and video
  ...
  \begin{tabular*}{\textwidth}{c @{\extracolsep{\fill}}lllllllllllll}
    ...
    \noalign{\smallskip}\bottomrule
    \\
    \multicolumn{13}{l}{Post-saccadic oscillations}\\
    \toprule\noalign{\smallskip}
    & \multicolumn{4}{l}{Images} & \multicolumn{4}{l}{Dots} & \multicolumn{4}{l}{Videos}\\
    Algorithm & Mean & SD & \# & rank &  Mean & SD & \# & rank & Mean & SD & \# & rank \\
    \noalign{\smallskip}\hline\noalign{\smallskip}
...
neural networks \citep{Startsev2018}. Such algorithms tend to require
substantial amount of (labeled) training data, which can be a critical
limitation in the context of a research study.
...
provided implementation of the \remodnav\ algorithm acknowledges this fact by
...
as well as the \LaTeX\ files necessary to compile the full PDF, are located in
a Github repository\footnote{https://github.com/psychoinformatics-de/paper-remodnav/}.
The necessary input data (from \cite{Andersson2017} and the
\textit{studyforrest.org} project) are installed as \texttt{datalad subdatasets},
so that updates or fixes of the source data can be pulled easily via
\texttt{datalad update}. The repository contains appropriate Makefiles, and
the full paper, including (automatically recomputed and inserted) results and
figures, can be regenerated with a single \texttt{make} call in the root of the
directory.
