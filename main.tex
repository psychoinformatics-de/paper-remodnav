...
\at Special Lab Non-Invasive Brain Imaging, Leibniz Institute for Neurobiology, Brenneckestra{\ss}e~6, 39118 Magde\-burg, Germany;
%Tel.: +123-45-678910\\
%Fax: +123-45-678910\\
...
guided by three objectives: 1) compare algorithm performance to other existing
approaches; 2) demonstrate plausible results on data from prolonged gaze
...
duration distribution characteristics (mean and standard deviation of
...
represents a scalar distribution similarity score that can be used as an
...
uses the scores for the human raters given in these original tables. As
...
type into zero-based ranks.

The LNS algorithm \cite{Larsson2013} was found to have the most human-like
...
\citet{Andersson2017} demonstrates that \remodnav\ yields the highest congruency
to manual human annotation in any stimulus category on a sample-by-sample basis
compared to ten contemporary algorithms when detecting fixations, saccades, and
post-saccadic oscillations. Its performance stays good and exceeds all other
...
performance equals or exceeds those of the original NH. Taking the number of
events and their duration distribution into account, REMoDNaVs performance
stays good.  Importantly, \remodnav\ performs similar to human coders in almost
all event categories and stimulus types. NH outperforms it only for fixation
detection in the image and video category, however, in these categories
\remodnav\ still classifies comparatively well. Thus, \remodnav\ does improve
the NH algorithm in aspects that were  suboptimal, but does not diminish
...
stimulation, as indicated by the previous comparison, we determined whether it is
...
bottom right panel in \fig{overallComp}. To compare the event detection
between high and lower quality samples, we computed the event distribution
...
In order to determine the robustness of \remodnav\ to noise, as a last step, we
applied it to the lower quality eye tracking dataset of the studyforrest
...
the same between the high and lower quality data sample, \remodnav\ demonstrates
robust performance if the characteristics of extracted events within the lower
quality dataset correspond to those obtained from the previous high quality data
sample.
...
\fig{remodnav} displays the performance of \remodnav\ for a lower
...
\fig{dist} displays the distribution parameters per event type for the lower
quality dataset in the bottom panel.
Altogether, \fig{remodnav}, \fig{dist} and
\fig{overallComp} illustrate a high similarity between performance in high
and lower quality datasets. In both cases, the main sequences follow the known
peak-velocity to amplitude relationship of saccades. It is notable that the
...
generally lower amount of events in the lower quality dataset. Given
that \remodnav\ classifies data from dynamic stimulation well, and that results
on lower and high quality data yield similar biologically plausible results, the
