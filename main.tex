...
stimuli \citep{real_world}. Using
dynamic stimuli promises to observe the nuances of cognition in a more life-like
...
algorithm had to support the classification of \textit{smooth pursuit} events, as
...
smooth pursuit classification]{LARSSON2015145,Komogortsev2013}. Following the suggestion
of \citet{hessels2018eye}, we define the different supported eye-movements
...
is surpressed (cite). \textit{Post-saccadic oscillations}, finally, are periods of
ocular instability after a saccade.
\todo{citations and language improvments for this part}


Here we introduce \remodnav\ (robust eye movement detection for natural
viewing), a new tool that aims to meet these objectives. It is built on the
...
Note, however, that some of the computational
components of eye movements can be adjusted to an individual researcher's own
definition by changing the algorithms' parameter values.
...
\todo[inline]{After validating that what I computed actually is what we need, we
need to interprete these values. Granted that direct comparisons between Kappa values
are not possible  because of the unclear computation of K in the paper,
we have higher or equal (tiny bit higher) values than NH on every stimulus and event type.
LNS has higher values for PSOs, we're equal with LNS on saccades.}
...
was 0.44 degrees of visual angle. (do we have these values for in-lab acquisition?)
...
\citep{Zemblys2018}. Such statistical learning algorithms often require a substantial amount of
